package com.sso.auth2.service.filter;
...
public class PermitAuthenticationFilter extends OAuth2AuthenticationProcessingFilter {
    ...
    public PermitAuthenticationFilter() {
        ...
        dt.setTokenStore(jwtTokenStore);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (authentication == null) {
            ...
            if(StringUtils.isNotBlank(accessToken)){
                ...
                try {
                    OAuth2AccessToken oAuth2AccessToken = jwtTokenStore.readAccessToken(headerToken.split(" ")[1]);
                    log.info("token =" +oAuth2AccessToken.getValue());
                }catch (InvalidTokenException e){
                    error.set(true);
                    map.put("message",e.getMessage());
